...
**Release decision (2026-08):** the rows still open below are **deferred past v1.0** — they remain this track's sourcing backlog, every affected page keeps its verify-the-exact-part hedge, and none of them gates the release.
...
| XT ratings from AMASS-direct hosting | Closed for values (audit-2026-07) via distributor-hosted AMASS PDFs; an AMASS-domain-hosted equivalent would still be an upgrade — **deferred post-1.0 (2026-08)** |
| Crimping tiny open-barrel terminals | Manufacturer application notes / crimp specifications. **Deferred post-1.0 (2026-08)** — the page's pre-crimped-leads guidance stands, and the general crimp mechanism is now cited (TE whitepapers; NASA-STD-8739.4 §12.3.3) |
