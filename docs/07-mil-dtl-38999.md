...
description: "MIL-DTL-38999 deep dive: series coupling styles, shell sizes, insert arrangements, contact test currents, keying, and decoding a real part number."
...
If you work around aerospace or defense hardware, you'll meet 38999 sooner or later. It packs sealing, high contact density, rugged coupling, EMI backshells, removable crimp contacts, alternate keying, and a mature multi-supplier ecosystem into one family. It also packs enough part-number fields and accessory choices to punish anyone who orders from the photo.

![A 38999-style square-flange panel receptacle with gold pin contacts next to its mating plug with knurled coupling nut](/img/photos/mil-dtl-38999-pair.jpg)

*A 38999-style pair: square-flange receptacle (left) and plug with coupling nut (right) — note the recessed gold crimp contacts and witness bands. Photo: [Abaillieul](https://commons.wikimedia.org/wiki/File:38999_connector_nickel-teflon.jpg), CC BY-SA 3.0, via Wikimedia Commons.*

:::tip[Just need to choose?]

For a scenario walkthrough, see the [Defense / rugged external I/O decision path](decision-paths/defense-rugged-external-io.md). This page is the detail behind that choice.
...
*What you're looking at on the bench: square-flange receptacle, knurled coupling nut, Tri-Start thread — and the ID cues that separate Series III from the bayonet series.*
...
Shell-size numbers are designations, not direct millimeter measurements. Approximate ODs are shell-style-dependent — for the Amphenol Series III straight plug, coupling-nut OD runs from ~22 mm (size 9) to ~48 mm (size 25), and receptacle/jam-nut styles are larger. Verify dimensions against the specific manufacturer datasheet.[^amphenolcat]
...
![Relative mating-end sizes of AS39029 contact sizes 22D, 20, 16, 12, and 8](/img/diagrams/38999-contact-sizes.svg)

*Approximate relative sizes, for orientation only — dimensions and ratings come from the AS39029 contact drawing and the derating curve.*
...
*The master keyway stays fixed; the minor keyways rotate per position. Angles shown are the shell-size 17/19 receptacle BSC values from the manufacturer's master-key table[^amphenolcat] — they differ by shell size, so decode from the catalog.*
