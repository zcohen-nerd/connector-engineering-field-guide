...
| 5 | Good vs bad open-barrel crimp diagram | **Done (2026-08)** — `hobby-crimp-good-vs-bad.svg` | Good crimp annotated (conductor wings on strands, insulation wings on jacket, tips just visible) + four wrongs in a 2×2 grid (strip too long / too short / stray strand / **two wires in a one-wire barrel**); embedded in `crimping.md` with a caption deferring texture to the photo backlog's macro set (P1 there still stands) |
| 6 | Power vs signal connector decision sketch | **Done (2026-08)** — `hobby-power-vs-signal.svg` | Three rows: mA signal path through a small connector (fine), amps through the same connector (heat at contact resistance), and the rated/keyed/fused power path; embedded in `power-vs-signal.md` |
| 7 | LED power injection connector warning diagram | **Done (2026-08)** — `hobby-led-power-injection.svg` | Data pigtail at the strip's input vs. thick injection feeds landing mid-strip/far-end through rated connectors, with the not-through-the-data-connector path crossed out; embedded in `jst-sm-led-connectors.md` |
